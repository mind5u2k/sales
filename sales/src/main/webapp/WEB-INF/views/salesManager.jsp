			...
			<div class="jarviswidget   jarviswidget-sortable" id="wid-id-1"
				...
				<header role="heading">
					...
					<button class="btn btn-primary" style="float: right;"
						onclick="addNewUserModel();">Add Sales Manager</button>
				</header>
				<div role="content">
								...
								<tbody>
									<c:forEach items="${salesManagers}" var="salesManager">
										...
										<tr role="row" class="odd">
											<td class="sorting_1">${salesManager.user.firstName}&nbsp;${salesManager.user.lastName}</td>
											<td>${salesManager.user.email}</td>
											<td>${salesManager.user.contactNumber}</td>
											<td>${salesManager.user.role}</td>
											<td
												style="text-align: center; padding: 0; vertical-align: middle;"><c:if
													test="${salesManager.user.enabled == true}">
												...
												</c:if> <c:if test="${salesManager.user.enabled == false}">
													<span class="onoffswitch"> <input type="checkbox"
