			...
			<div class="jarviswidget   jarviswidget-sortable" id="wid-id-1"
				...
				<header role="heading">
					...
					<button class="btn btn-primary" style="float: right;"
						onclick="addNewUserModel();">Add Client</button>
				</header>
				<div role="content">
								...
								<tbody>
									<c:forEach items="${clients}" var="client">
										...
										<tr role="row" class="odd">
											<td class="sorting_1">${client.user.firstName}&nbsp;${client.user.lastName}</td>
											<td>${client.user.email}</td>
											<td>${client.user.contactNumber}</td>
											<td>${client.user.role}</td>
											<td
												style="text-align: center; padding: 0; vertical-align: middle;"><c:if
													test="${client.user.enabled == true}">
												...
												</c:if> <c:if test="${client.user.enabled == false}">
													<span class="onoffswitch"> <input type="checkbox"
